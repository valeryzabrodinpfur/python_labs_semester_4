...
class TestValidateTransaction:
    ...
    @pytest.mark.parametrize(
        "id_val, amount_val, category_val, date_val, currency_val, "
        "expected_exception",
        [
            # Валидные случаи
            ...
            # Неверная валюта
            ("tx8", 100.0, "Books", "2025-01-01", "USD",
    ...
    def test_validate_transaction_parametrized(
        self, id_val, amount_val, category_val, date_val, currency_val,
        expected_exception
    ):
        tx = Transaction(
            id=id_val, amount=amount_val,
            category=category_val, date=date_val, currency=currency_val
        )
    ...
    def test_currency_mismatch_custom_message(self):
        tx = Transaction(
            id="tx", amount=100.0, category="Test",
            date="2025-01-01", currency="EUR"
        )
        with pytest.raises(
            CurrencyMismatchError,
            match="Ожидалась валюта RUB, получена EUR"
        ):
            validate_transaction(tx, expected_currency="RUB")
